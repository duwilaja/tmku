...
if($mn=='ticket'){
	...
	if($code=='200'){
		//save history
		$up=upload_file("fattc","attc/");
		$attc=$up[0]&&$up[1]!=''?$up[1]:'';
		$sql=sql_insert("xtm_notes","ticketno,lastnote,status",$conn,"updatedon,updatedby,attc","now(),'$s_ID','$attc'");
		$rs=exec_qry($conn,$sql);
...
if($mn=='ticketx'){
	...
	if($code=='200'){
		...
		$attc=$up[0]&&$up[1]!=''?$up[1]:'';
		$dtl=post('detail');
		$sql=sql_insert("xtm_notes","rowid",$conn,"ticketno,lastnote,status,updatedon,updatedby,attc","$tn,'$dtl','new',now(),'$s_ID','$attc'");
		$rs=exec_qry($conn,$sql);
		if(db_error($conn)!=""){ $msgs="Failed saving history".db_error($conn);}
	}
}
